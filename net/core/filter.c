...
int skb_do_redirect(struct sk_buff *skb)
{
	 ...
	 * Sockets with a refcount (sk_is_refcounted or sock_pfree_cnt)
...
BPF_CALL_3(bpf_sk_assign, struct sk_buff *, skb, struct sock *, sk, u64, flags)
{
	 ...
	 * Use sock_pfree_cnt so the release path knows a refcount is held.
	 */
	...
	skb_orphan(skb);
	skb->sk = sk;
	skb->destructor = egress_rcu_sk ? sock_pfree_cnt : sock_pfree;
